...
from torch.distributed.distributed_c10d import NCCL_SHRINK_DEFAULT, NCCL_SHRINK_ABORT
...
class ProcessGroupNCCLGroupTest(MultiProcessTestCase):
    ...
    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(int(os.environ.get("WORLD_SIZE", "1")) < 2, "Test requires world size >= 2")
    ...
    def test_shrink_group_backend_properties(self):
        ...
        log_test_info(self.rank, f"Excluding ranks: {ranks_to_exclude}, am_excluded: {is_excluded}")
        ...
        if not is_excluded:
            ...
            log_test_info(self.rank, f"Storing original backend properties: timeout={original_timeout}, high_priority={original_high_priority}")

        if is_excluded:
            log_test_info(self.rank, f"Excluded rank {self.rank} - setup complete, skipping shrink operation")
            dist.destroy_process_group() # hang without it
            return

        # Only non-excluded ranks proceed with shrink (same as _perform_shrink_test)
        log_test_info(self.rank, f"Non-excluded rank calling shrink_group")
        ...
        log_test_info(self.rank, f"Timeout comparison - original: {original_timeout}, new: {new_timeout}")
        self.assertEqual(original_timeout, new_timeout, f"{test_name}: timeout not preserved")

        log_test_info(self.rank, f"High priority stream comparison - original: {original_high_priority}, new: {new_high_priority}")
        self.assertEqual(original_high_priority, new_high_priority, f"{test_name}: high_priority_stream not preserved")

        log_test_validation(self.rank, f"{test_name}: Backend properties preserved successfully")
        log_test_success(self.rank, f"{test_name} successful (shrink + backend validation)")
    ...
    def test_shrink_group_multiple_comms(self):
        ...
        if self.rank not in ranks_to_exclude:
            ...
            expected_sum = sum(range(self.world_size - 1))  # 0 + 1 + ... + (world_size-2)
    ...
    def _test_shrink_group_with_flag(self, shrink_flag, flag_name, rank_to_exclude):
        ...
        if flag_name == "NCCL_SHRINK_ABORT":
            log_test_info(self.rank, "ABORT flag will terminate ongoing operations before shrinking")

        self._perform_shrink_test(ranks_to_exclude, f"{flag_name} flag test", shrink_flags=shrink_flag)
    ...
    def test_shrink_group_performance(self):
        ...
        if not is_excluded:
            ...
            result = self._test_collective_on_shrunk_group(shrunk_pg, device, ranks_to_exclude, "performance")
    ...
    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(int(os.environ.get("WORLD_SIZE", "1")) < 4, "Test requires world size >= 4")
    @skip_if_lt_x_gpu(int(os.environ.get("WORLD_SIZE", "1")))
    def test_shrink_group_multiple_exclusions(self):
        ...
        ranks_to_exclude = [i for i in range(2, self.world_size, 2)]  # Every other rank from 2

        self._perform_shrink_test(ranks_to_exclude, "Multiple exclusions test")

    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(int(os.environ.get("WORLD_SIZE", "1")) < 3, "Test requires world size >= 3")
    @skip_if_lt_x_gpu(int(os.environ.get("WORLD_SIZE", "1")))
    def test_shrink_group_multiple_iterations(self):
        """Test multiple shrink operations in sequence."""
        log_test_info(self.rank, f"Starting test_shrink_group_multiple_iterations with world_size={self.world_size}")
        ...
        if self.world_size >= 6:
            first_exclusion.append(self.world_size - 2)  # Exclude last two ranks for larger sizes
        ...
        if self.rank not in first_exclusion:
            ...
            log_test_info(self.rank, f"After first shrinking: world_size {self.world_size} -> {current_world_size}")
            ...
            if current_world_size >= 3:
                second_exclusion = [current_world_size - 1]  # Exclude the new "last" rank
                log_test_info(self.rank, f"Second shrinking from group of size {current_world_size}: excluding ranks {second_exclusion}")
                ...
                if self.rank not in second_exclusion:
                    ...
                    log_test_info(self.rank, f"After second shrinking: world_size {current_world_size} -> {final_world_size}")
                    self.assertEqual(second_pg.size(), expected_final_size)

                    # Test collective on final group
                    tensor = torch.full((1,), self.rank).cuda(device)
                    log_test_info(self.rank, f"Performing all_reduce on final group (size {final_world_size}) with tensor: {tensor.item()}")
                    c10d.all_reduce(tensor, group=second_pg)
                    log_test_info(self.rank, f"Final all_reduce completed, result: {tensor.item()}")

                    # Calculate expected sum of remaining ranks
                    all_excluded = set(first_exclusion + second_exclusion)
                    remaining_ranks = [r for r in range(self.world_size) if r not in all_excluded]
                    expected_sum = sum(remaining_ranks)
                    log_test_info(self.rank, f"Remaining ranks: {remaining_ranks}, expected sum: {expected_sum}, actual: {tensor.item()}")
                    self.assertEqual(tensor.item(), expected_sum)
                    log_test_info(self.rank, "Final verification passed")
                else:
                    log_test_info(self.rank, "This rank excluded in second shrinking, not calling shrink_group")
            else:
                log_test_info(self.rank, "Skipping second shrinking (remaining group too small)")
        else:
            log_test_info(self.rank, "This rank excluded in first shrinking, not calling shrink_group")
    ...
    def _validate_shrunk_group(self, shrunk_pg, expected_size, test_name=""):
        ...
        self.assertEqual(actual_size, expected_size, f"{test_name}: group size mismatch")

        new_rank = shrunk_pg.rank()
        self.assertTrue(0 <= new_rank < expected_size, f"{test_name}: invalid new rank {new_rank}")

        log_test_info(self.rank, f"{test_name}: world_size {self.world_size} -> {actual_size}, rank {self.rank} -> {new_rank}")
        return new_rank

    def _test_collective_on_shrunk_group(self, shrunk_pg, device, ranks_to_exclude, test_name=""):
        ...
        expected_sum = sum(r for r in range(self.world_size) if r not in ranks_to_exclude)

        self.assertEqual(result, expected_sum, f"{test_name}: collective result mismatch")
        log_test_info(self.rank, f"{test_name}: collective passed ({result} == {expected_sum})")
        return result

    def _perform_shrink_test(self, ranks_to_exclude, test_name, shrink_flags=0, with_collective=True):
        ...
        if is_excluded:
            ...
            if shrink_flags & NCCL_SHRINK_ABORT:
                ...
                log_test_info(self.rank, f"cleanup resources for excluded rank {self.rank}")
                dist.destroy_process_group()
                log_test_info(self.rank, f"Excluded rank {self.rank} - exit")
            else:
                log_test_info(self.rank, f"Using regular destroy for excluded rank {self.rank}")
                dist.destroy_process_group()
            return None

        # Only non-excluded ranks proceed with shrink
        log_test_info(self.rank, f"Non-excluded rank calling shrink_group (default_group={is_default_group})")
        shrunk_pg = c10d.shrink_group(ranks_to_exclude, shrink_flags=shrink_flags)
        log_test_info(self.rank, f"Non-excluded rank calling shrink_group (default_group={is_default_group}) done")
        ...
        if with_collective:
            result = self._test_collective_on_shrunk_group(shrunk_pg, device, ranks_to_exclude, test_name)
    ...
    def test_shrink_group_vs_abort_reinit_performance(self):
        ...
        if not is_excluded:
            ...
            start_time = time.perf_counter()
            shrunk_pg = c10d.shrink_group(ranks_to_exclude, shrink_flags=NCCL_SHRINK_ABORT)
            ...
            # Verify correctness
            expected_sum = sum(r for r in range(self.world_size) if r not in ranks_to_exclude)
            self.assertEqual(test_tensor.item(), expected_sum, "shrink_test: collective result mismatch")
